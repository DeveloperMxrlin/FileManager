...
public class PluginUtil {
    ...
    public static Plugin getPluginByName(String[] args, int start) {
        return getPluginByName(args[start]);
    }
    ...
    public static String load(String name) {
        ...
        if (!pluginDir.isDirectory()) {
            return "Plugin Directory is not a Directory.";
        }
        ...
        if (!pluginFile.isFile()) {
            for (File f : pluginDir.listFiles()) {
                if (f.getName().endsWith(".jar")) {
                    try {
                        PluginDescriptionFile desc = FileManager.getInstance().getPluginLoader().getPluginDescription(f);
                        if (desc.getName().equalsIgnoreCase(name)) {
                            pluginFile = f;
                            break;
                        }
                    } catch (InvalidDescriptionException e) {
                        return "Can't find plugin.yml of plugin.";
                    }
                }
            }
        }
    ...
    public static String unload(Plugin plugin) {
        ...
        if (pluginManager != null) {
            ...
            try {
                ...
                e.printStackTrace();
                return "Failed unloading the plugin.";
        ...
        return "Successfully unloaded the plugin.";
